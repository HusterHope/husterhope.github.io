...
稍用一些计算机视觉的知识，我们可以快速提取到棋盘格平面上的每个「角点」。由于这些点全部处在同一平面上，不妨设它们的坐标为$(X,Y,0)$，因而我们可以直接删去$[X,Y,Z,1]$中的$Z$坐标与$[R\quad t]$中的第三列($r_{13},r_{23},r_{33}$)，将新的$[R\quad t]$矩阵记作矩阵$H$，称为同质变换。（会在第二周的第二节课讲到）
